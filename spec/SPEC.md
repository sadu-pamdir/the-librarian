...
**Claim types and their standards.** A *formal* claim is judged by proof (2 + 2 = 5: refuted, the hardest status). An *empirical* claim is judged by studies and reproduction ("the MMR vaccine causes autism": refuted per the Lancet full retraction 2010 and Taylor et al. 2014). An *experiential* claim is judged by documented practice under stated conditions (expert intuition: supported *within* domains that give fast, reliable feedback; Kahneman & Klein 2009). An *interpretive* claim is a map, not a territory, judged by coherence and usefulness ("attested", deliberately weaker than "supported").
...
- **Bidirectional honesty** *(v0.2)*: the system can say "this is documented as false, refuted by X" with the same auditability as any positive answer; plain RAG over the known-true cannot express this.

## 6. Outlook (position, not claim)

Three convergence theses motivate where this spec wants to run eventually. We state them as positions:

1. **Memory-centric hardware.** Single-level stores (TreeSLS), processing-in-memory, and unified memory are converging toward machines where the historical RAM/storage split disappears. Since February 2026 the industry supplies direct evidence: SanDisk and SK hynix are standardizing High Bandwidth Flash (HBF) under the Open Compute Project: 3D-NAND stacked on a logic base die, HBM-class read bandwidth (~1.6 TB/s per stack, vendor figure) at 8–16× HBM capacity; first samples H2 2026. (CXL pursues the same convergence from the opposite direction, disaggregating memory into shared pools, though its fate in the AI era is contested.) HBF works precisely because inference weights are read-mostly: NAND endurance is consumed by program/erase cycles, not reads; read disturb exists but is controller-managed and costs no endurance, so the classic endurance objection loses its force for this workload. Note the drift, however: HBF *adds a tier* (HBM ↔ HBF ↔ SSD) where this position argues for *dissolving tiers*. The industry is converging on the destination while still building staircases. Uncompressed weights of a 1.2-trillion-parameter model occupy ≈ 2.4 TB in BF16, feasible on today's high-capacity portable storage, though a true PIM "stick" does not exist yet; current memristor arrays are lab-scale.
...
3. **The missing architecture.** The ingredients for a truly unified memory already exist, scattered across foreign contexts. Wavelength-multiplexed photonic in-memory computing performs the multiplication *inside* non-volatile phase-change cells, with independent data streams multiplexed on parallel wavelengths through the same waveguide (Feldmann et al., Nature 2021); demonstrated arrays remain lab-scale, single-digit dimensions, bounded by optical loss. Glass storage holds ~2 TB on a 2 mm plate for ~10,000 years (Project Silica; ordinary borosilicate since Feb 2026), but is write-once and slow: light wins at movement and parallelism, electronics at density. Recognition-based retrieval, memory addressed by content rather than location, has run in carrier-grade routers and switches for decades (TCAM), and its theoretical root, associative memory, was Nobel-recognized in 2024 (Hopfield, Physics). No single substrate dissolves the RAM/storage split alone: photonics lacks density, glass lacks write speed, content-addressable memory lacks capacity. What is missing is not a material but an architecture that thinks them together: not in competition, but in cooperation, working with one another, for humanity and for everything that lives on Earth. That is not naive; it is strategy. Game theory has known it since Axelrod's tournaments: in repeated encounters, the strategies that cooperate outperform the ones that exploit. Dense, persistent, recognizing, computing in place. The Librarian is written for that machine: its knowledge layer is read-mostly by design, navigated by meaning rather than by physical location (a map, not an address table), and indifferent to which substrate finally wins.
